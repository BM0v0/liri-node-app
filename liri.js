...
function getTweets() {
	console.log("Locating tweets.\n");
	var twitter = new Twitter(keys.twitterKeys);
	twitter.get('statuses/home_timeline', function(error, tweets, response) {
		if (tweets.length < 1) {
			console.log("No tweets from your account. Try again and tweeted something.");
			return;
		}
		if (tweets.length === 1) {
...
function spotifySong(song) {
	...
	.then(function(response) {
		...
		if (!foundSong) {
			console.log("Couldn't find '" + song + "' on Spotify.");
		}
	})
	.catch(function(err) {
	...
	request(queryUrl, function (error, response, body) {
		...
		if (body && JSON.parse(body).Search && JSON.parse(body).Search.length > 0) {
			...
			request(innerQueryURL, function (error, response, body) {
				...
				if (body && JSON.parse(body) && JSON.parse(body).Response === "True") {
					body = JSON.parse(body);
					console.log("Can't find it but can this do?:\n");
					console.log("  Title: " + body.Title);
					console.log("  Year: " + body.Year);
					for (var j = 0; j < body.Ratings.length; j++) {
						if (body.Ratings[j].Source === "Internet Movie Database") {
			...
			console.log("No movies called '" + movie + "'. Make sure to use the exact name of the movie!");
		}
	});
};
...
function parseFileCommand() {
	fs.readFile("random.txt", "utf8", function(error, data) {
		if (error) {
			return console.log("Wait1 error.\n  " + error);
		}
